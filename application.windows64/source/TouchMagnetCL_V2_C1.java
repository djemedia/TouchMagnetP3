...
public class TouchMagnetCL_V2_C1 extends PApplet {
...
class stainedglassRenderer extends AudioRenderer {
 ...
 stainedglassRenderer(AudioSource source) {
  ...
  public void setup() {
    ...
    grad.addColorAt(0,NamedColor.MEDIUMVIOLETRED);
    //grad.addColorAt(64,NamedColor.DARKORANGE);
    //grad.addColorAt(86,NamedColor.BLACK);
    //grad.addColorAt(88,NamedColor.PURPLE);
    grad.addColorAt(128,NamedColor.INDIGO);
    grad.addColorAt(192,NamedColor.DEEPSKYBLUE);
    grad.addColorAt(215,NamedColor.PURPLE);
    grad.addColorAt(255,NamedColor.MEDIUMVIOLETRED);
    }
    ...
    grad.addColorAt(224,NamedColor.MAROON);
    grad.addColorAt(192, NamedColor.BLACK);
    ...
    */
    ...
    grad.addColorAt(128, NamedColor.BLACK);
    //grad.addColorAt(192, NamedColor.BROWN);
  ...
  public void draw () {
    for (int x = 0; x < velocity.length; x++) {
      for (int y = 0; y < velocity[x].length; y++) {
       ...
       //color 
        float setContrastModeF = (float)map(vFader4, 0, 255, 0, .00004f); 
...
class FitzhughRenderer extends AudioRenderer {
  ...
  */
  ...
  public void draw() {
    ...
    pixels[j*w+i] = color(((setcolorMode+.1f)*gridU[i][j]+setcolorMode-vFader4-20), vFader2,gridU[i][j]*vFader3);  
  }
    }
    updatePixels();
  ...
  */  
  ...
  public synchronized void draw()
  {
    ...
    if (mousePressed && (mouseButton == LEFT))
      apply_heat(mouseX, mouseY, 25, .25f);
    if (mousePressed && (mouseButton == RIGHT))
      apply_heat(mouseX, mouseY, 25, -.2f);
  ...
  public void onClick(float mX, float mY) {
    ...
    int oY = (int)cY;
    if (toggle == true)
    apply_heat(oX, oY, 25, .15f);
    if (toggle == false)
    apply_heat(oX, oY, 25, -.2f);
  }
...
/*
 ...
 // random angular offset
  ...
  public void setup() {
    ...
    //int setResF = (int)map(vFader6, 0, 255, 2, 12);
     int setScaleDetailF = 1;
    int setResF = 1;
 ...
 //crashes when mouseclicks by border
    ...
    for(int x=0; x<w; x++)
      for(int y=0; y<h; y++) {
        ...
        float setContrastF = (float)map(vFader4, 0, 255, 0.001f, .1f); 
        ...
        switch(palette) {
         ...
         //          
          ...
          case 0: img.pixels[c] = color(setcolorMode + 20 * sin(val*setContrastF), vFader2, val + 127 * sin(val*.0004f));  break;
          case 1: img.pixels[c] = color(setcolorMode + 20 * sin(val*setContrastF), vFader2, vFader3 - val);  break;
          case 2: img.pixels[c] = color(setcolorMode + 20 * sin(val*setContrastF), vFader2, val + vFader3 * sin(val*.0004f));  break;
...
/*
   ...
   internalX = 234;
   internalY = 63;//starting point on canvas
...
//  float setcolorMode = 200;
  ...
  class Particle {
    ...
    public void update() {
      ...
      float speed = (float)map(vFader5, 0, 255, 0, 3);
      x -= speed * cos(angle);
      y += speed * sin(angle);
...
class Particle
{
  ...
  public void update()
  {
    ...
    int setDetailF = (int)map(vFader6, 0, 255, 1, 10);
    ...
    velocity.x = setSpeed*(noise(noisefield.oX/setDetailF+position.y/100)-0.5f);
    velocity.y = setSpeed*(noise(noisefield.oY/setDetailF+position.x/100)-0.5f);
  ...
  public void render()
  {
    ...
    stroke((setcolorMode)*sin(HALF_PI), vFader2, vFader3);
...
public synchronized void draw()
{
  ...
  for (int y = 0; y < height; ++y)
  {
    for (int x = 0; x < width; ++x)
    {
     ...
     float setContrastModeF = (float)map(vFader4, 0, 255, 0, .6f);
     float setSpeedModeF = (float)map(vFader5, 0, 255, .00001f, .0008f);
     float setNoiseDetailF = (float)map(vFader6, 0, 255, .0001f, .01f);
...
class RadarRenderer extends AudioRenderer {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "TouchMagnetCL_V2_C1" };
    if (passedArgs != null) {
