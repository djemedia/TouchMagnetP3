 ...
 //crashes when mouseclicks by border
    ...
    for(int x=0; x<w; x++)
      for(int y=0; y<h; y++) {
        ...
        float setBrightnessF = (float)map(vFader3, 0, 255, .0001, .1);
        float setContrastF = (float)map(vFader4, 0, 255, 0.001, .1); 
        ...
        switch(palette) {
         ...
         //          
          ...
          case 0: img.pixels[c] = color(setcolorMode + 20 * sin(val*setContrastF), vFader2, val + 127 * sin(val*.0004));  break;
          case 1: img.pixels[c] = color(setcolorMode + 20 * sin(val*setContrastF), vFader2, vFader3 - val);  break;
          case 2: img.pixels[c] = color(setcolorMode + 20 * sin(val*setContrastF), vFader2, val + vFader3 * sin(val*.0004));  break;
